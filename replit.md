...
- July 16, 2025. Created comprehensive Support page with professional contact form and FAQ section. Implemented email notification system that sends support requests to info@contractclarity.co.uk and confirmation emails to users. Enhanced email service to support multiple SMTP options: Gmail SMTP (with GMAIL_USER/GMAIL_APP_PASSWORD), custom SMTP (EMAIL_HOST/EMAIL_PORT/EMAIL_USER/EMAIL_PASS), and development fallback that logs emails to console. Added Support link to navigation menu and built complete support request workflow with validation and error handling.
