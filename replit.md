...
- July 12, 2025. Implemented automatic Stripe subscription cancellation on account deletion. When users delete their accounts, all active and trialing Stripe subscriptions are automatically cancelled to prevent continued billing. Enhanced account deletion endpoint with comprehensive subscription cleanup including error handling to ensure user data removal even if Stripe operations fail.
...
- July 12, 2025. Implemented Option 10C as the new official Contract Clarity logo based on user's 80% similar reference design. Features layered contract documents with purple checkmark on white background for brand consistency. Updated main logo component across entire application with the finalized design including purple gradient documents, document text lines, signature area, and purple branded checkmark symbol.
- July 13, 2025. Added comprehensive multilingual support for Usage Status component across all supported languages. Implemented string interpolation functionality in translation system to handle dynamic content like page limits. Enhanced translation keys for usage status including: title, contract analyses used, document size limit, various status messages (limit reached, near limit, available, payment required), upgrade prompts, and internationalized button text. All usage status content now displays properly in English, Spanish, French, German, and Arabic with proper cultural context and RTL support.
...
- July 16, 2025. Added SocialFooter component to all main pages (home, about, landing) with actual social media links (@contractclarity Instagram, @contractcl X/Twitter, @contractclarity TikTok). Created comprehensive Support page with contact form that sends emails to info@contractclarity.co.uk. Enhanced email service with support email functionality including professional HTML templates.
- July 16, 2025. Created comprehensive Privacy Policy and Terms of Service pages with strong legal disclaimers emphasizing that Contract Clarity is NOT a replacement for legal advice and users should always seek professional legal counsel. Added routing for /privacy-policy and /terms-of-service. Updated footer links across the application to point to these legal pages. Emphasized user responsibility to read contracts thoroughly and consult qualified attorneys.
