...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/billing/history', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const billingHistory = invoices.data.map(invoice => ({
        ...
        description: `${invoice.lines.data[0]?.description || 'Subscription'} - ${new Date(invoice.created * 1000).toLocaleDateString()}`,
      }));
  ...
  app.post('/api/subscription/reactivate', isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (!subscription) {
        return res.status(400).json({ error: 'No subscription set for cancellation' });
      }
